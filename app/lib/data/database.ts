...
import { Collection, Document, IndexDescriptionInfo, ObjectId } from "mongodb";
...
export class Database {
    ...
    createDocument = async (dbName: string, collectionName: string, document: Document) => {
        if (!isValidCollectionName(collectionName)) {
            return Promise.reject(new Error(`The collection name "${collectionName} is invalid`));
        }
        const ndb = this.connectionData.getClient()?.db(dbName);
        const collection = ndb?.collection(collectionName);
    ...
    getCollections = async (dbName: string): Promise<Collection<Document>[] | undefined> => {
        const collections = await this.connectionData.getClient()?.db(dbName).collections();
        return collections;
    };

    getDocuments = async (dbName: string, collectionName: string, query: object = {}, limit: number = 50, skip: number = 0): Promise<Document[]> => {
        if (!isValidCollectionName(collectionName)) {
            return Promise.reject(new Error(`The collection name "${collectionName} is invalid`));
        }
        const ndb = this.connectionData.getClient()?.db(dbName);
        const collection = ndb?.collection(collectionName);
        if (!collection) {
            return Promise.reject(new Error(`The collection name "${collectionName}" does not exist in the database "${dbName}"`));
        }
