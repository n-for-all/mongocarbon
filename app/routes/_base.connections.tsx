import { ActionableNotification, AccordionItem, Accordion, Layer, Modal, Button } from "@carbon/react";
		...
		<>
			<Layer className="w-full max-w-md p-6 mx-auto my-16 mb-16 bg-white border border-solid shadow-lg border-neutral-200">
				...
				<Accordion>
					{connections.map((connection: DbConnection) => (
						<AccordionItem key={connection.id} title={connection.name} className="bg-neutral-100">
							<h3 className="mb-2 text-xl font-medium">{connection.name}</h3>
			...
			{error && (
				<div className="fixed -translate-x-1/2 left-1/2 bottom-10">
					<ActionableNotification title="Error" subtitle={error} closeOnEscape inline={false} onClose={() => setError(null)} />
				</div>
			)}
			<ConnectionModal
